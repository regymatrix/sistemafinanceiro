...
namespace PrjFinanceiro.Controllers
{
    ...
    public class BairroApiController : ControllerBase
    {
        ...
        public IActionResult BuscarPorId(int id)
        {
            ...
            if (bairro == null)
            {
                return NotFound(new { message = $"Bairro com código {id} não encontrada." }); // HTTP 404
            }
        ...
        public IActionResult Remover(int id)
        {
            ...
            if (bairro == null)
            {
                return NotFound(new { message = $"Bairro de código {id} não encontrada para exclusão." });
            }

            _context.Bairro.Remove(bairro);
            _context.SaveChanges();

            return Ok(new { success = true, message = "Bairro excluída com sucesso diretamente pela API!" }); // HTTP 200
        }
    }
}
