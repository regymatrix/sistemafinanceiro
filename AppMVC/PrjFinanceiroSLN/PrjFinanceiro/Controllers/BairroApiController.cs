...
namespace PrjFinanceiro.Controllers
{
    ...
    public class BairroApiController : ControllerBase
    {
        ...
        public IActionResult Cadastrar([FromBody] BairroCreateDto dto)
        {
            ...
            // Validação da FK: Verifica se a cidade informada realmente existe no banco
            var cidadeExiste = _context.Cidade.Any(c => c.Codigo == dto.CodigoCidade);
            if (!cidadeExiste)
            {
                return BadRequest(new { message = $"A FK CodigoCidade {dto.CodigoCidade} informada não existe na tabela Cidade." });
            }

            var novoBairro = new Bairro
            {
                NomeBairro = dto.NomeBairro,
                CodigoCidade = dto.CodigoCidade // Chave Estrangeira (FK)
            };
        ...
        public IActionResult Atualizar(int id, [FromBody] BairroCreateDto dto)
        {
            ...
            // Validação da FK para atualização
            var cidadeExiste = _context.Cidade.Any(c => c.Codigo == dto.CodigoCidade);
            if (!cidadeExiste)
            {
                return BadRequest(new { message = $"A FK CodigoCidade {dto.CodigoCidade} informada não existe na tabela Cidade." });
        ...
        public IActionResult Remover(int id)
        {
            ...
            return Ok(new { success = true, message = "Bairro excluído com sucesso diretamente pela API!" });
        }
    }
}