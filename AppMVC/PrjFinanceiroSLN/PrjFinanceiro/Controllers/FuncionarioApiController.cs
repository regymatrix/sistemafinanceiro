...
namespace PrjFinanceiro.Controllers
{
    ...
    public class FuncionarioApiController : ControllerBase
    {
        ...
        public IActionResult BuscarPorId(int id)
        {
            ...
            if (funcionario == null)
            {
                return NotFound(new { message = $"Funcionario com código {id} não encontrado." }); // HTTP 404
            }
        ...
        public IActionResult Atualizar(int id, [FromBody] FuncionarioCreateDto dto)
        {
            ...
            {
                return NotFound(new { message = $"Funcionario de código {id} inexistente para atualização." });
            }
        ...
        public IActionResult Remover(int id)
        {
            ...
            if (funcionario == null)
            {
                return NotFound(new { message = $"Funcionario de código {id} não encontrada para exclusão." });
            }

            _context.Funcionario.Remove(funcionario);
            _context.SaveChanges();

            return Ok(new { success = true, message = "Funcionario excluída com sucesso diretamente pela API!" }); // HTTP 200
        }
    }
}
